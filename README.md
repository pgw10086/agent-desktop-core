# `@platform/desktop-core`

这是可供多个 Product Desktop 使用的中性 Desktop Core 平台 package。它只提供经过类型约束的桌面能力、
Electron 适配器、Surface/快捷键/通知/deadline 生命周期、DSH 进程监管和受信 IPC 入口，不包含任何产品的
主窗口布局、业务插件装配、DSH 包选择或 DSH 私有实现。
...
`@platform/desktop-core/electron-notification-factory` 子路径。
